...
impl SessionParser {
  pub fn from_file(
    ...
    Ok(Self::parse(&content, view))
  }

  pub fn parse(content: &str, view: &Arc<dyn ViewTrait + Send + Sync>) -> SessionStats {
    ...
    for (line_index, line) in content.lines().enumerate() {
      ...
      if let Some((ts, rest)) = line.split_once(']') {
        ...
        Self::handle_line(rest.trim(), &mut stats);
      }
    }
  ...
  fn handle_line(line: &str, stats: &mut SessionStats) {
    if line.contains(DEAUTH_TOKEN) {
      ...
      Self::handle_epoch_line(line, &mut stats.epochs);
    } else if line.contains(PEER_TOKEN) {
      Self::handle_peer_line(line, &mut stats.peers);
    }
  }
