...
use crate::{
  ...
  traits::ui::ViewTrait, utils::general::format_duration_human,
};
...
const START_TOKEN: &str = r"connecting to http";
...
static LAST_SESSION_FILE: LazyLock<String> =
  LazyLock::new(|| config().debug.last_session_file.to_string());
...
impl Default for LastSession {
  fn default() -> Self {
    Self {
      path: Cow::Borrowed(&config().debug.last_session_file),
...
impl LastSession {
  ...
  pub fn parse(&mut self, ui: &dyn ViewTrait, skip: Option<bool>) {
