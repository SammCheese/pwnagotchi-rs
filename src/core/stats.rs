  ...
  agent
    .execute(|agent| {
      ...
      if agent.current_channel == 0 {
        agent.automata.view.set("aps", StateValue::Text(tot_aps.to_string()));

        agent.automata.view.set("sta", StateValue::Text(tot_stas.to_string()));
      } else {
    ...
    agent
      .execute(move |agent| {
        agent.automata.view.set(
          "uptime",
          StateValue::Text(format_duration_human(now.duration_since(started).unwrap_or_default())),
        );
      })
      .await;
  }
