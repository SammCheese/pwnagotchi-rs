...
pub async fn start_sessionfetcher(sm: &Arc<SessionManager>, view: &Arc<View>) {
...
async fn update_handshakes(sm: &Arc<SessionManager>, view: &Arc<View>) {
  let handshakes_path = config().main.handshakes_path.to_string();
...
async fn update_aps(sm: &Arc<SessionManager>, view: &Arc<View>) {
  let session = sm.get_session().await;
  let state = session.state.read();
...
async fn update_uptime(sm: &Arc<SessionManager>, view: &Arc<View>) {
