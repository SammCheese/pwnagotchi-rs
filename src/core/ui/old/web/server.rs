...
use tera::{ Tera };
use crate::core::{ config::config, log::LOGGER, ui::old::web::routes::{ inbox_handler, index_handler, ui} };
...
pub async fn static_handler(Path(path): Path<String>) -> Response {
    ...
    STATIC_ASSETS.get_file(path).map_or_else(|| (StatusCode::NOT_FOUND, format!("File not found: {path}")).into_response(), |file| {
...
fn build_tera_from_include_dir() -> Tera {
  ...
  tera.add_raw_templates(templates).expect("Failed to load templates");
  tera
}
