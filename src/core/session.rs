...
use crate::core::{ agent::Peer, config::config, log::LOGGER, mesh::advertiser::Advertisement, ui::view::View, utils::format_duration_human, voice::Voice };
...
const PEER_PARSER: &str =
  r"detected unit (.+)@(.+) \(v.+\) on channel \d+ \(([\d\-]+) dBm\) \[sid:(.+) pwnd_tot:(\d+) uptime:(\d+)]";

const LAST_SESSION_FILE: &str = "/home/sammy/Public/pwnagotchi-rs/test/.pwnagotchi-last-session";
...
impl LastSession {
  ...
  pub fn parse(&mut self, ui: &View, skip: Option<bool>) {
    ...
    match fs::read_to_string(&self.path) {
      Ok(content) => {
        let content_lines: Vec<String> = content.lines().map(|line| line.trim().to_string()).rev().collect();
        ...
        LOGGER.log_debug("Session", &format!("Parsing last session logs: {:?}", lines.len()));
        self.parse_stats();
        self.parsed = true;
      }
      Err(e) => {
  ...
  fn parse_stats(&mut self) {
    ...
    for line in self.last_session.clone() {
      ...
      stopped_at = chrono::NaiveDateTime
        ::parse_from_str(line_timestamp, "%Y-%m-%d %H:%M:%S")
        .ok()
  ...
  fn handle_epoch_line(
    ...
    if let Some(caps) = epoch_re.captures(line) {
      ...
      for cap in epoch_data_re.captures_iter(epoch_data) {
        ...
        if
          let (Some(key), Some(value)) = (key, value) &&
          key.as_str() == "reward" &&
